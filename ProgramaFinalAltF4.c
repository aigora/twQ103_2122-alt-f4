...
f1=fopen("marcador.txt", "w");
if(f1==NULL){
    printf("Error de apertura de la base de datos\n");
    return 0;
}

for(i=0;i<njug;i++){
    fprintf(f1, "%s %d %d", jugadores[i].nombre, jugadores[i].monedas, jugadores[i].ronda=0);
}
color(hConsole, 10);
...
for(i=0;i<njug;i++){
    jugadores[i].ronda++;
    fprintf(f1, "%s %d", jugadores[i].nombre, jugadores[i].monedas, jugadores[i].ronda);
}
...
for(i=0;i<njug;i++){
    jugadores[i].ronda++;
    fprintf(f1, "%s %d\n", jugadores[i].nombre, jugadores[i].monedas, jugadores[i].ronda);
}
...
case 2:
    ...
    f1 = fopen("marcador.txt", "r");
    if(f1 == NULL){
        printf("Error de lectura en la base de datos.\n");
    }
    while(fscanf(f1,"%s %d %d", jugadores[i].nombre, &jugadores[i].monedas, &jugadores[i].ronda)!=EOF){
        i++;
        cont=i;
        }

    printf("Datos recuperados:\n");
    printf("Seguiremos por la siguiente pregunta:\n\n");
    fclose(f1);
    f1=fopen("marcador.txt", "w");
    if(f1==NULL){
...
for(i=0;i<njug;i++){
   if(jugadores[i].monedas>0)
    ...
    printf("La partida anterior se termino, por favor inicie una nueva partida.\n");
    fclose(f1);
    return 0;
    break;
}
